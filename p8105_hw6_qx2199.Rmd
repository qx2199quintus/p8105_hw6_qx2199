...
fit model using a stepwise method: 
...
```
According to the average RMSE value and the violin plot,  my  model ( `babysex` + `bhead` + `blength`+ `delwt`+ `fincome` +`gaweeks`+ `mheight`+ `mrace`+`parity`+ `ppwt`+` smoken`  ) has the lowes residuals. Therefore, it's the best model among the three.The model with only main effects ( `blength` +`gaweeks` ) has the highest residual and hence is the worst one.The model with  length, sex, and all interactions the medium residuals among the three models. Therefore, it's the second best model. 
