...
```
From the residual plot, we could see that most of the residuals are not heavily skewed,especially when the predicted value is higher than 2000g. The true residual value are around slightly more than 3000. However, when the predicted values are smaller than 2000, there are outliers; when the predicted value decrease, residuals increase.
...
```{r}
 weather_final %>% 
  ...
  geom_density()
```
The log(beta0 *beta1) plot,it's approximately normal-distributed, with t with peak that around 2.012
